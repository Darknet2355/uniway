...
# ─────────────────────────────────────────────────────────────────────────────
GEMINI_API_KEY = "AIzaSyBBYDMxG2b5IHJeqr7vHBKjtF4JBEjoXZ0"
GEMINI_URL = (
...
# ─────────────────────────────────────────────────────────────────────────────
#  CAMPUS KNOWLEDGE  — used by both AI endpoints
# ─────────────────────────────────────────────────────────────────────────────
CAMPUS_CONTEXT = """
You are the official AI assistant for Kyambogo University (KYU) campus in Kampala, Uganda.
Answer ONLY questions about Kyambogo University — locations, directions, facilities, services.
For unrelated questions, politely redirect the user.
...
ADMINISTRATION:
- Administration Block (Senate): collect student ID, Registrar, Finance — open Mon-Fri 8am-5pm
- Guild Offices: student government, next to Admin Block
- Registrar's Office & Finance Department: inside Administration Block

LIBRARIES:
- Central Library (Main): thousands of textbooks/journals, borrow up to 3 books — Mon-Sat 8am-8pm
- E-Library: computers & internet for digital research, right next to Central Library
- Faculty of Engineering Library: inside Engineering building

FACULTIES & SCHOOLS (west side): Engineering, Science, Arts & Humanities, Vocational Studies, Health Sciences, Built Environment
SCHOOLS (central/east): Education, Management, Law

DINING:
- Main Cafeteria: central campus, 3 meals/day, busiest 12pm-2pm
- Faculty canteens: Engineering Canteen, Science Canteen (quieter, closer to classes)
- Student Market: east side near hostels, stationery, toiletries, snacks

BANKING:
- Bank (Stanbic) & Bank (Centenary): east side near Student Market — good for student accounts

MEDICAL:
- University Health Centre: general health, Mon-Fri 8am-5pm
- Dental Clinic & Pharmacy: right next to Health Centre
- After hours emergency: Police Post near Eastern Gate (24/7)

RELIGIOUS: Chapel (St. Francis) & Mosque — east side, open to all students

STUDENT HOSTELS (east side, near Eastern Gate):
- Girls Hostel Block A & B, Boys Hostel Block C & D, International Students Hostel
- All have 24hr security and study areas

SPORTS (west side): Sports Ground (Main), Basketball Court, Volleyball Court, Tennis Court, University Gym
— join sports clubs early, inter-faculty competitions each semester

OTHER:
- ICT Center: computer workstations, internet access
- Printing Press: printing, binding, photocopying — best for large jobs like final year projects
- University Bookshop: textbooks and stationery
- Police Post: near Eastern Gate, 24/7
- University Farm: far west side

NAVIGATION TIPS:
- Campus is walkable end-to-end in ~20 minutes
- Main tarmac road runs through centre from Main Gate
- Use landmarks: "pass the cafeteria", "turn at Admin Block"

RESPONSE RULES:
- Be warm, concise, mobile-friendly
- Use short bullet points for lists of places or steps
- When your answer is about a specific navigable destination, end with exactly:
  [NAVIGATE:ExactDestinationName]
- Use ONLY these exact names inside [NAVIGATE:...]:
...
def gemini_generate(prompt_text, system_text=None):
    """
    Call Gemini 1.5 Flash (free tier).
    Returns the response text or raises an exception.
    """
    # Combine system instruction + user prompt if system given
...
def gemini_chat(messages, system_text):
    """
    Multi-turn chat using Gemini.
    `messages` is a list of {role, content} dicts (OpenAI-style from frontend).
    Gemini uses 'user'/'model' roles — we map 'assistant' → 'model'.
    """
    # Prepend system context as the first user turn + model ack
    contents = [
        {"role": "user",  "parts": [{"text": f"[SYSTEM CONTEXT]\n{system_text}"}]},
        {"role": "model", "parts": [{"text": "Understood. I am the KYU AI Campus Assistant. How can I help?"}]},
    ]
    for m in messages:
...
def api_assistant():
    """Multi-turn chat for the AI Campus Assistant page."""
    data     = request.get_json()
    messages = data.get("messages", [])
    if not messages:
        return jsonify({"error": "No messages provided"}), 400
    try:
        reply = gemini_chat(messages, CAMPUS_CONTEXT)
...
def api_ai_tip():
    """Personalised one-tip per First Day Guide tour stop."""
    data    = request.get_json()
    faculty = data.get("faculty", "your faculty")
    name    = data.get("name",    "Fresher")
    stop    = data.get("stop",    "this location")
    prompt  = (
        f"You are a friendly senior student at Kyambogo University. "
        f"Write ONE short practical tip (max 2 sentences, no intro phrase) about "
        f'"{stop}" that is specifically useful for a {faculty} student named {name}. '
        f"Be specific and accurate to Kyambogo University campus in Kampala, Uganda."
    )
    try:
        tip = gemini_generate(prompt)
        return jsonify({"tip": tip.strip()})
    except Exception:
        return jsonify({"tip": ""}), 200   # JS uses the hardcoded fallback text


# ─────────────────────────────────────────────────────────────────────────────
#  ROUTING ENDPOINT  (OSRM walking, with straight-line fallback)
# ─────────────────────────────────────────────────────────────────────────────
...
def api_route():
    ...
    # ── Try OSRM (free, open-source, no key needed) ──────────────────────────
    try:
        osrm_url = (
